...
void basel_problem_decoupled_scan(benchmark::State &state) {
    ...
    using namespace parallel_primitives;
    using T = float;
    auto in = sycl::malloc_device<T>(state.range(0), q);
    auto out = sycl::malloc_device<T>(state.range(0), q);

    for (auto _ : state) {
        decoupled_scan_device<scan_type::inclusive, sycl::plus<>>(q, in, out, state.range(0));
    }
    state.SetBytesProcessed(sizeof(T) * state.iterations() * state.range(0));
    sycl::free(in, q);
    sycl::free(out, q);
